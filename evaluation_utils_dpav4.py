...
def shuffle_all(predictions, plaintexts, keys, offsets):
    ...
    return predictions, plaintexts, keys, offsets


def get_log_prob(predictions, plaintext, mask, offset):
    predictions = np.squeeze(predictions)
    n_classes = predictions.shape[0]
    keys = np.arange(n_classes, dtype=int)
    x_xor_k = np.bitwise_xor(keys, plaintext)
    tz = np.take(sbox, x_xor_k)
    z = np.bitwise_xor(tz, mask[(offset+1)%16])
    log_prob = np.take(predictions, z)

    return log_prob


def compute_key_rank(predictions, plaintexts, keys, mask, offsets):
    n_samples, n_classes = predictions.shape
    plaintexts = plaintexts[:n_samples]
    keys = keys[:n_samples]
    offsets = offsets[:n_samples]
    predictions, plaintexts, keys, offsets = shuffle_all(predictions, plaintexts, keys, offsets)
    ...
    for i in range(n_samples):
        log_prob = get_log_prob(predictions[i, :], plaintexts[i], mask, offsets[i])
